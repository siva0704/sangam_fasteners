...
const Products = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden max-w-full bg-slate-50 dark:bg-[#050814] font-sans transition-colors duration-500">
      <Seo
      ...
      <main className="flex-1">
        ...
        <section className="py-8 bg-white dark:bg-[#0a0f1c] border-b-2 border-slate-200 dark:border-slate-800 sticky top-0 z-30 transition-colors duration-500 shadow-[0_10px_30px_rgba(0,0,0,0.05)] dark:shadow-[0_10px_30px_rgba(0,0,0,0.4)]">
          <div className="container mx-auto px-4 max-w-7xl">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
              <div>
                <h2 className="text-3xl font-black font-heading text-slate-900 dark:text-white uppercase tracking-tight">
                  Product Catalog
                </h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-mono uppercase tracking-widest">Filter by category, material type, or industry standard.</p>
              </div>

              <div className="flex items-center gap-3 bg-slate-100 dark:bg-[#0f172a] px-5 py-3 border-2 border-slate-200 dark:border-slate-800">
                <Filter size={18} className="text-cyan-500" />
                <span className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-[0.2em]">Found <strong className="text-cyan-600 dark:text-cyan-400 text-sm">{filteredProducts.length}</strong> items</span>
              </div>
            </div>

            <div className="grid grid-cols-2 md:flex md:flex-wrap items-end gap-4 pt-4 border-t-2 border-slate-100 dark:border-slate-800/50">
              {/* Category Filter */}
              <div className="flex flex-col gap-2 flex-1 md:flex-none">
                <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">Category</span>
                <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                  <SelectTrigger className="w-full md:w-[220px] h-12 bg-white dark:bg-[#0b1221] border-2 border-slate-200 dark:border-slate-700 rounded-none shadow-[4px_4px_0px_rgba(15,23,42,0.05)] dark:shadow-[4px_4px_0px_rgba(6,182,212,0.1)] focus:ring-0 focus:border-cyan-500 hover:border-cyan-500 focus:shadow-none transition-all dark:text-white font-bold uppercase text-xs tracking-wider">
                    <SelectValue placeholder="All Categories" />
                  </SelectTrigger>
                  <SelectContent className="rounded-none border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-[#0b1221] shadow-[8px_8px_0px_rgba(15,23,42,0.1)] dark:shadow-[8px_8px_0px_rgba(6,182,212,0.2)] dark:text-white">
                    {categories.map(cat => (
                      <SelectItem key={cat} value={cat} className="uppercase text-xs font-bold tracking-wider hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer">{cat === 'all' ? 'All' : cat}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              {/* Material Filter */}
              <div className="flex flex-col gap-2 flex-1 md:flex-none">
                <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">Material</span>
                <Select value={materialFilter} onValueChange={setMaterialFilter}>
                  ...
                  <SelectContent className="rounded-none border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-[#0b1221] shadow-[8px_8px_0px_rgba(15,23,42,0.1)] dark:shadow-[8px_8px_0px_rgba(6,182,212,0.2)] dark:text-white">
                    {materials.map(mat => (
                      <SelectItem key={mat} value={mat} className="uppercase text-xs font-bold tracking-wider hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer">{mat === 'all' ? 'All' : mat}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              {/* Standard Filter */}
              <div className="flex flex-col gap-2 flex-1 md:flex-none">
                <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">Standard</span>
                <Select value={standardFilter} onValueChange={setStandardFilter}>
                  ...
                  <SelectContent className="rounded-none border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-[#0b1221] shadow-[8px_8px_0px_rgba(15,23,42,0.1)] dark:shadow-[8px_8px_0px_rgba(6,182,212,0.2)] dark:text-white">
                    {standards.map(std => (
                      <SelectItem key={std} value={std} className="uppercase text-xs font-bold tracking-wider hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer">{std === 'all' ? 'All' : std}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
        ...
        {/* Products Grid */}
        <section className="py-16 bg-gradient-to-b from-slate-50 to-white dark:from-[#0a0f1c] dark:to-[#0f172a] overflow-x-hidden min-h-[50vh] transition-colors duration-500 pb-32">
          ...
          <div className="container mx-auto px-4 max-w-7xl relative z-10">
            {filteredProducts.length === 0 ? (
              <div className="text-center py-32 border-2 border-dashed border-slate-300 dark:border-slate-700">
                <Target className="w-12 h-12 text-slate-400 dark:text-slate-600 mx-auto mb-4" />
                <h3 className="text-xl font-bold font-heading text-slate-900 dark:text-white uppercase">No Products Found</h3>
              ...
              >
                {filteredProducts.map((product, idx) => (
                  <div key={product.id} className="group relative flex flex-col bg-white dark:bg-[#0b1221] border-2 border-slate-200 dark:border-slate-800 rounded-none overflow-hidden h-full transition-all duration-300 hover:border-cyan-500 dark:hover:border-cyan-500 hover:shadow-[8px_8px_0px_rgba(15,23,42,0.1)] dark:hover:shadow-[8px_8px_0px_rgba(6,182,212,0.2)] hover:-translate-y-1 isolate">

                    {/* Image Section - Clickable via Link */}
                    <Link to={`/products/${product.id}`} className="block relative h-64 bg-slate-50 dark:bg-[#0f172a] flex items-center justify-center p-8 overflow-hidden border-b-2 border-slate-100 dark:border-slate-800/50 cursor-pointer">
                      ...
                      <div className="absolute top-4 right-4 z-20">
                        <span className="text-[10px] font-bold text-slate-800 dark:text-slate-300 bg-slate-200/50 dark:bg-slate-800/50 px-2 py-1 rounded-sm shadow-inner uppercase tracking-wider backdrop-blur-sm">
                    ...
                    <div className="p-8 pt-6 flex-1 flex flex-col relative z-20">
                      <div className="mb-5">
                        ...
                        <h3 className="text-xl md:text-2xl font-black font-heading text-slate-900 dark:text-white uppercase transition-colors duration-300 leading-tight">
                          {product.name}
                        </h3>
                      </div>
                      ...
                      <div className="mt-auto pt-6 border-t-2 border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <Link to={`/products/${product.id}`} className="group/link flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors uppercase text-xs font-bold tracking-widest">
