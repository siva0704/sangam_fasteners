                    ...
                    {certifications.map((cert, idx) => {
                        ...
                        return (
                            <AnimatedSection key={idx} animation="fade-up" delay={idx * 0.1}>
                                <div className="relative isolate overflow-hidden bg-white dark:bg-card/50 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-border hover:shadow-xl dark:hover:shadow-blue-900/20 transition-all duration-500 hover:-translate-y-1 h-full flex flex-col items-center text-center group hover:border-blue-100 dark:hover:border-blue-500/30">
                                    ...
                                    <div className={`relative z-10 w-16 h-16 rounded-full bg-slate-50 dark:bg-background flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-white dark:group-hover:bg-card group-hover:shadow-md transition-all duration-300 text-slate-500 dark:text-muted-foreground group-hover:text-blue-600 dark:group-hover:text-blue-400`}>
                                        <cert.icon className="w-8 h-8 group-hover:rotate-12 transition-transform duration-300" />
                                    </div>
                                    <h3 className="relative z-10 text-xl font-bold font-heading text-slate-900 dark:text-foreground mb-3 group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors duration-300">
                                        {cert.title}
                                    </h3>
                                    <p className="relative z-10 text-sm text-slate-500 dark:text-muted-foreground leading-relaxed mb-auto group-hover:text-slate-600 dark:group-hover:text-muted-foreground transition-colors duration-300">
                                        {cert.desc}
                                    </p>
                                    <div className="relative z-10 mt-6 pt-6 border-t border-slate-100 dark:border-border w-full flex justify-center group-hover:border-blue-200/50 dark:group-hover:border-blue-500/30 transition-colors">
                                        <div className="flex items-center gap-2 text-xs font-medium text-slate-500 dark:text-muted-foreground group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
