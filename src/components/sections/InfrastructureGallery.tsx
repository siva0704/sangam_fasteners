...
export const InfrastructureGallery = () => {
    ...
    return (
        <section className="py-24 bg-secondary dark:bg-background relative transition-colors duration-500 overflow-hidden">
            {/* Ambient glows */}
            ...
            <div className="container mx-auto px-4 relative z-10">
                <AnimatedSection animation="fade-up" className="mb-16 text-center">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary/50 dark:bg-foreground/5 border border-border mb-6">
                        <Settings2 className="w-4 h-4 text-primary" />
                        <span className="text-xs font-bold uppercase tracking-widest text-foreground dark:text-muted-foreground">Our Capabilities</span>
                    </div>
                    <h2 className="text-4xl md:text-5xl font-heading font-extrabold text-foreground mb-6">Machinery & Instruments</h2>
                ...
                <div className="max-w-6xl mx-auto mb-12">
                    <div className="relative group max-w-md mx-auto">
                        ...
                        <Input
                            ...
                            className="pl-12 py-6 rounded-full border-border bg-background dark:bg-foreground/5 backdrop-blur-md focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-all text-foreground"
                            value={searchTerm}
                ...
                <Tabs defaultValue="machinery" className="max-w-7xl mx-auto">
                    <div className="flex justify-center mb-12">
                        <TabsList className="bg-secondary dark:bg-foreground/5 p-1 rounded-full border border-border h-auto">
                            <TabsTrigger
                                value="machinery"
                    ...
                    <TabsContent value="machinery">
                        ...
                        {filterList(machineryList).length === 0 && (
                            <div className="text-center py-20 text-muted-foreground">No machinery found matching your search.</div>
                        )}
                    </TabsContent>
                    ...
                    <TabsContent value="instruments">
                        ...
                        {filterList(instrumentList).length === 0 && (
                            <div className="text-center py-20 text-muted-foreground">No instruments found matching your search.</div>
                        )}
                    </TabsContent>
                </Tabs>
            </div>
...
const EquipmentCard = ({ item, idx }: { item: InfrastructureItem; idx: number }) => {
    return (
        <AnimatedSection animation="scale-in" delay={idx * 0.02} className="h-full">
            <div className="group relative h-full flex flex-col p-6 rounded-2xl bg-background dark:bg-card border border-border hover:border-blue-500/50 dark:hover:border-blue-400/50 hover:shadow-[0_15px_30px_-10px_rgba(59,130,246,0.3)] transition-all duration-300">
                <div className="flex justify-between items-start mb-4">
                    ...
                    {item.year && (
                        <span className="text-[10px] font-bold bg-secondary dark:bg-foreground/5 px-2 py-1 rounded text-muted-foreground">
                            Y.O.I: {item.year}
                        </span>
                    )}
                </div>

                <h3 className="text-sm font-bold text-foreground leading-tight mb-3 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors uppercase">
                    {item.name}
                </h3>
                ...
                <div className="mt-auto pt-4 border-t border-border">
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs font-bold text-foreground dark:text-muted-foreground tracking-wide">{item.make === 'MANUAL' ? 'Hand Operated' : item.make}</span>
                        </div>
                        {item.details && (
                            <div className="flex flex-col text-right">
                                <span className="text-[10px] uppercase tracking-wider text-muted-foreground dark:text-slate-500 font-bold">SPECS</span>
                                <span className="text-xs font-medium text-muted-foreground">{item.details}</span>
                            </div>
                        )}
                    </div>
                </div>
