            ...
            <div className="container mx-auto px-4 relative z-10">
                <AnimatedSection animation="fade-up" className="mb-16 text-center">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 mb-6">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest text-slate-700 dark:text-slate-300">Our Capabilities</span>
                    </div>
                    <h2 className="text-4xl md:text-5xl font-heading font-extrabold text-slate-900 dark:text-white mb-6">Machinery & Instruments</h2>
                ...
                <div className="max-w-6xl mx-auto mb-12">
                    <div className="relative group max-w-md mx-auto">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-blue-600 dark:group-focus-within:text-blue-400 transition-colors" />
                        <Input
                            type="text"
                            placeholder="Search equipment or brand..."
                            className="pl-12 py-6 rounded-full border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 backdrop-blur-md focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-all text-slate-900 dark:text-white"
                            value={searchTerm}
                ...
                <Tabs defaultValue="machinery" className="max-w-7xl mx-auto">
                    <div className="flex justify-center mb-12">
                        <TabsList className="bg-slate-100 dark:bg-white/5 p-1 rounded-full border border-slate-200 dark:border-white/10 h-auto">
                            <TabsTrigger
                                value="machinery"
                                className="rounded-full px-8 py-3 data-[state=active]:bg-blue-600 data-[state=active]:text-white transition-all font-bold text-sm tracking-wide"
                            >
...
const EquipmentCard = ({ item, idx }: { item: InfrastructureItem; idx: number }) => {
    return (
        <AnimatedSection animation="scale-in" delay={idx * 0.02} className="h-full">
            <div className="group relative h-full flex flex-col p-6 rounded-2xl bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/5 hover:border-blue-500/50 dark:hover:border-blue-400/50 hover:shadow-[0_15px_30px_-10px_rgba(59,130,246,0.3)] transition-all duration-300">
                <div className="flex justify-between items-start mb-4">
                    ...
                    {item.year && (
                        <span className="text-[10px] font-bold bg-slate-100 dark:bg-white/10 px-2 py-1 rounded text-slate-500 dark:text-slate-400">
                            Y.O.I: {item.year}
                        </span>
                    )}
                </div>

                <h3 className="text-sm font-bold text-slate-900 dark:text-white leading-tight mb-3 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors uppercase">
                    {item.name}
                </h3>
                ...
                <div className="mt-auto pt-4 border-t border-slate-100 dark:border-white/5">
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                            <span className="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 font-bold">MAKE</span>
                            <span className="text-xs font-bold text-slate-700 dark:text-slate-300 tracking-wide">{item.make === 'MANUAL' ? 'Hand Operated' : item.make}</span>
                        </div>
                        {item.details && (
                            <div className="flex flex-col text-right">
                                <span className="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 font-bold">SPECS</span>
                                <span className="text-xs font-medium text-slate-600 dark:text-slate-400">{item.details}</span>
                            </div>
                        )}
                    </div>
                </div>
