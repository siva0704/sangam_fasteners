                    ...
                    {showcaseProducts.map((product, idx) => (
                        <AnimatedSection key={product.id} animation="fade-up" delay={idx * 0.1}>
                            <div className="relative group flex flex-col bg-white rounded-xl border border-slate-200 hover:border-blue-500/30 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-500 overflow-hidden h-full md:min-h-[450px]">
                                ...
                                <Link to={`/products/${product.id}`} className="block relative h-28 md:h-64 overflow-hidden bg-slate-50/50 flex items-center justify-center border-b border-slate-100 group-hover:bg-white transition-colors duration-500 cursor-pointer">
                                    {/* Technical Badge */}
                                    ...
                                    <div className="absolute top-2 right-2 z-10">
                                        <span className="text-[9px] font-bold text-slate-700 bg-white border border-slate-200 px-1.5 py-0.5 rounded-sm shadow-sm uppercase tracking-wider backdrop-blur-sm">
                                            {product.category}
                                        </span>
                                    </div>
                                ...
                                {/* Content Section */}
                                <div className="p-2 md:p-6 flex-1 flex flex-col relative z-10 bg-white">
                                    <div className="mb-2 md:mb-4">
                                        <h3 className="text-xs md:text-xl font-bold font-heading text-slate-900 group-hover:text-blue-600 transition-colors mb-2 leading-tight line-clamp-2">
                                            {product.name}
                                        </h3>
                                        <div className="w-6 md:w-10 h-0.5 bg-slate-100 group-hover:bg-blue-500 transition-colors duration-500" />
                                    </div>

                                    <p className="text-slate-500 text-[10px] md:text-sm leading-relaxed mb-3 md:mb-6 line-clamp-2">
                                        {product.description}
                                    </p>

                                    <div className="mt-auto flex flex-row items-center justify-between gap-1.5 pt-2 md:pt-5 border-t border-slate-50">
                                        <Link to={`/products/${product.id}`} className="group/link flex items-center gap-1 text-slate-400 hover:text-blue-600 transition-colors">
                                            ...
                                            <span className="md:hidden text-[9px] font-bold uppercase tracking-wider">Specs</span>
                                        ...
                                        <div onClick={(e) => e.preventDefault()} className="relative z-20">
                                            <div className="[&>button]:bg-slate-900 [&>button]:text-white [&>button]:hover:bg-blue-600 [&>button]:transition-all [&>button]:duration-300 [&>button]:h-7 md:[&>button]:h-9 [&>button]:text-[9px] md:[&>button]:text-xs [&>button]:uppercase [&>button]:tracking-wide [&>button]:font-bold [&>button]:px-3 md:[&>button]:px-5 [&>button]:shadow-md [&>button]:hover:shadow-blue-500/25 [&>button]:w-auto">
