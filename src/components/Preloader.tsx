...
const Bolt = () => {
    ...
    const logoTexture = useTexture(`${import.meta.env.BASE_URL}logo.png`);
...
const Preloader = ({ onComplete }: { onComplete: () => void }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[100] bg-background transition-opacity duration-1000 ease-in-out ${isExiting ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
            <Suspense fallback={null}>
                <Canvas dpr={[1, 2]} gl={{ antialias: false, toneMapping: THREE.ReinhardToneMapping, toneMappingExposure: 1.5, alpha: true }}>
                    <Scene />
                </Canvas>
            </Suspense>
            ...
            <div className={`absolute bottom-10 left-1/2 -translate-x-1/2 text-center transition-opacity duration-500 ${isExiting ? "opacity-0" : "opacity-100"}`}>
                <div className="h-0.5 w-32 bg-cyan-500/50 mx-auto rounded-full mb-4 animate-pulse" />
                <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-[0.4em]">Initializing Systems</p>
            </div>
        </div>
    );
};
